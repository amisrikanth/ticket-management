...
namespace ticket_management.Controllers
{
    ...
    public class TicketsController : ControllerBase
    {
        ...
        // PUT: api/Tickets/5
        [HttpPut("{id}")]
        public async Task<IActionResult> EditTicket([FromQuery]string ticketid, [FromQuery] string status, [FromQuery] string priority, [FromQuery] string intent)
        {
            await _ticketService.EditTicket(ticketid, status, priority, intent);
            return Ok();
        }
